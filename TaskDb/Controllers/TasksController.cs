...
public class TasksController : ControllerBase {
    ...
    public async Task<ActionResult<TaskItem>> GetById(int id) {
        ...
        if(task is null)
            return NotFound(new {Message = $"Задача с аййди {id} нне найдена"});
        return Ok(task);
    }
    ...
    public async Task<IActionResult> Update(int id, [FromBody] UpdateTaskDto dto) {
        ...
        if (string.IsNullOrWhiteSpace(dto.Title))
            return BadRequest(new { Message = "Поле Title не быть пустым" });
    ...
    public async Task<ActionResult<TaskItem>> ToggleComplete(int id) {
        var task = await _db.Tasks.FindAsync(id);
        if(task is null)
            return NotFound(new {Message = $"Задача с айди {id} не найденна"});
    ...
    public async Task<ActionResult> Delete(int id) {
        var task = await _db.Tasks.FindAsync(id);
        if(task is null)
            return NotFound(new {Message = $"Задача с if {id} не найденна"});
